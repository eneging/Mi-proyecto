...
import { useContador } from '../components/hooks/useContador';
...
function Buscador() {
  ...
  //contador y otros hooks
  const [titulo, cambiarTitulo] = useTitle('Whole Finland');

  const [contador, sumar, restar] = useContador(0);

  const [contador2, sumar2, restar2] = useContador(0);

  const Guests = contador + contador2;
...
setToggleState(index);
  ...
  const handleSubmit = (e) => {
    ...
    const Myfilter = data.filter((el) => el.city === titulo && el.maxGuests >= Guests)
  ...
  return (
    ...
    <>
      ...
      <nav>
        ...
        <svg width="130" height="40" viewBox="0 0 96 19" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div >
          ...
          <button className="Boton1" onClick={() => (setMostrar(true) ,toggleTab(1)
          )}>
            {titulo}</button>

          <button className="Boton" onClick={() => (setMostrar(true), toggleTab(2))} >
            <div> Guests </div>{Guests}</button>

          <button className="search-nav" onClick={() => (setMostrar(true), toggleTab(1))} >
            <span className="material-symbols-outlined" id="lupa">
              search</span></button>

        </div>
      </nav>

      {mostrar ? <div className="container-form">


        <div className="edit"><strong>Edit your search</strong> <button className="remove" onClick={() => setMostrar(false)}><span className="material-symbols-outlined">
          close
        </span></button>  </div>
        <div className="contenedor">
          ...
          <form onSubmit={handleSubmit}>
            <div className='barra-form'>
              ...
              <button className='tabs active-tabs'  id='tabs active-tabs' type="button" onClick={ () => toggleTab(1)}>
                <div><strong>Location</strong></div>
                <div><input type='text' value={titulo} readOnly
                ></input></div>
              </button>

              <button   id="btn-Guets" type="button" onClick={ () => toggleTab(2)}>
        ...
        <div className='continer-btn'>
          ...
          <div className= { toggleState === 1 ? 'btn-ciudades active-content' : 'content'} >


            <ul className='btn-paises' >

              <li> <button className='button-pais' type='button' onClick={
                () => cambiarTitulo('Helsinki')
              } >

                <span className="material-symbols-outlined">location_on </span>
                <span className="span-locatiob">
                  Helsinki, Country
                </span></button>
              </li>
              <li> <button className='button-pais' type='button' onClick={
              ...
              </li><li> <button className='button-pais' type='button' onClick={
                () => cambiarTitulo('Vaasa')
              }>
          ...
          <div className= {toggleState === 2 ? 'btn-add-number': 'content'} id='#addNumber'>
      ...
      <div className="state" ><h2>Stays in Finland</h2> {filtered.length} stays </div>
      <Cards filter={filtered}></Cards>
